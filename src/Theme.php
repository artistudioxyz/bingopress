...
class Theme
{
	...
	public function __construct()
	{
		...
		$this->WP = new WordPress\Helper\Helper();
		/** Init Config */
		...
		$this->config->path =
			...
			'.php';
		$this->config->options = get_option('bingopress_config');
	...
	public function loadModulesHooks()
	{
		...
		foreach (
			$this->Helper->getDirFiles(
				$this->path['framework_path'] . 'src/Helper/BingoPressModule'
			)
			as $module
		) {
		...
		foreach ($controllers as $controller) {
			...
			foreach ($controller->getHooks() as $hook) {
				...
				if (in_array(strtolower($class), $this->enableHooks)) {
					...
					$status = isset($this->config->options->bingopress_hooks->$key)
						? $this->config->options->bingopress_hooks->$key
	...
	public function setDefaultOption()
	{
		...
		if (empty($config) || !$config) {
			...
			update_option('bingopress_config', (object) $config);
