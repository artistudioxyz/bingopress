...
<div class="bingopress-container">
	...
	<div class="header bg-white shadow-sm rounded-lg mt-6 mr-4">
		<nav class="flex flex-wrap items-center px-4 py-2 border-b-1 border-gray-100" role="navigation">
			...
			<div class="md:w-auto md:flex-grow md:flex md:items-center absolute display-inline-block float-right right-0">
				<ul class="bingopress-menu-desktop nav-tab-wrapper <?php echo ( isset( $disableTab ) ) ? '' : 'nav-tab-general'; ?> <?php echo isset( $active ) ? 'tab-active' : ''; ?>
				hidden sm:inline-flex flex-row cursor-pointer mx-4 pt-3">
					<?php foreach ( $this->sections as $bingopress_path => $section ) : ?>
						<?php
							extract( $this->sectionLoopLogic( $bingopress_path, $section ) );
							$bingopress_type = ! strpos( $tab, 'href' ) ? 'nav-nonurl' : '';
						?>
						<li
							class="px-8 py-2 first:border-0 border-l-2 border-gray-100
							<?php echo esc_attr( $bingopress_type ); ?> <?php echo ( $active ) ? 'tab-active' : ''; ?>"
				...
				<div class="sm:hidden inline-block float-right right-0">
					<div class="bingopress-menu-responsive mx-4 pr-3">
                                    ...
                                    ?>
										<div class="py-1">
											<div data-tab="section-<?php echo esc_attr( $slug ); ?>" class="menu-item cursor-pointer text-gray-700 flex justify-between w-full px-4 py-2 text-sm leading-5 text-left"  role="menuitem" >
                                                <?php
                                                    /** Tab contains 2 types: External Link (https://google.com) and Internal Link (#setting) */
                                                    echo strpos( $tab, '//' ) ? esc_url( $tab ) : esc_html( $tab );
                                                ?>
											</div>
										</div>
								<?php endforeach; ?>
	...
	<div class="content py-4 mr-4">
		<?php foreach ( $this->sections as $bingopress_path => $section ) : ?>
			...
			<div id="section-<?php echo esc_attr( $slug ); ?>" class="tab-content bingopress-sections <?php echo ( $active ) ? 'current' : ''; ?>">
				<?php
